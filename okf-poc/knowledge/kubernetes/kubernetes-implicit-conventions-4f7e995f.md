---
id: kubernetes-implicit-conventions-4f7e995f
type: concept
title: Implicit conventions
description: 'There are some implicit conventions worth noting here:'
category: kubernetes
tags: []
source:
  name: kubernetes
...
- The scheduler only considers nodes that have all `topologySpreadConstraints[*].topologyKey` present at the same time.
  ...
  1. any Pods located on those bypassed nodes do not impact `maxSkew` calculation - in the
     above [example](https://kubernetes.io/docs/concepts/scheduling-eviction/topology-spread-constraints/#example-conflicting-topologyspreadconstraints), suppose the node `node1`
