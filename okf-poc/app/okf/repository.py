...
`settings.KNOWLEDGE_DIR` (`knowledge/` by default) is the canonical source of
truth. Every concept is a Markdown file with YAML frontmatter. This module
reads, validates and searches that repository directly from the filesystem, so
the vector database (Qdrant) is only an acceleration layer (Phases 7 & 9), and
`data/raw/` (crawled HTML, uploaded raw files) is only a disposable cache that
feeds the pipeline which produces these concept files.
"""
...
def load_all_concepts(knowledge_dir: Optional[str] = None, use_cache: bool = True) -> list[OKFConceptFile]:
    """
    Load and validate every concept from the knowledge repository (the single
    canonical `settings.KNOWLEDGE_DIR` root, or an explicit override).
